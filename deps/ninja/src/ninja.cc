...
#ifndef NINJA_BUILD_LIB
    #define NINJA_BUILD_LIB 1
#endif
...
struct ninja_runtime {
    ...
    // ninja_runtime() : main(nullptr, config) {
    //     config.parallelism = GuessParallelism(); g_metrics = new Metrics();
    // }
...
ninja_runtime::ninja_runtime() : main(nullptr, config) {
    config.parallelism = GuessParallelism(); g_metrics = new Metrics();
}

// ninja_runtime __attribute__((init_priority(1000))) $ninja;
extern "C" char $ninja[sizeof(ninja_runtime)] = {0};
#else
